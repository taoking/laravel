...
通知系统需要事件触发、数据库通知、未读数、实时推送、聚合和队列异步发送。

它涉及的核心组件是：Laravel Notification、database channel、broadcast channel、Redis unread、Queue、notifications 表。

## 2. 解决的问题

- 用户层面：用户能收到评论、点赞、关注、开播、私信和系统通知。
- 系统层面：把业务事件转化为可读、可追踪、可聚合的用户通知。
- 不做的问题：用户无法感知互动反馈，关注开播和评论回复等闭环断开。
- 所在位置：它连接视频互动、评论、关注、直播开播和私信。

## 3. 常见方案对比

| 方案 | 核心思路 | 需要组件 | 优点 | 缺点 | 适合阶段 | 是否推荐 |
| --- | --- | --- | --- | --- | --- | --- |
| 只写站内通知 | 使用 Laravel database notification | MySQL | 简单稳妥 | 不实时 | 学习版 | 推荐起步 |
| 数据库 + 未读数 | 通知入库并维护 unread_count | MySQL、Redis | 体验完整 | 要处理一致性 | 学习项目 | 推荐 |
| 广播实时通知 | database 后再 broadcast 给用户 | Reverb、private channel | 实时性好 | 多节点复杂 | 项目展示版 | 推荐升级 |
| 多渠道通知平台 | 站内、邮件、短信、Push 聚合 | 队列、第三方服务 | 生产成熟 | 成本和退订复杂 | 生产环境 | 后续可选 |
...
```text
方案名称：Laravel database notification + Redis 未读数 + private channel 可选实时
核心组件：Notification、Queue、Redis、Reverb
Laravel 职责：监听业务事件、生成通知、标记已读、查询通知中心
中间件职责：队列异步写通知和发送广播，Redis 缓存未读数
数据流：业务事件 -> Listener -> Notification 入库 -> 增未读 -> 可选 broadcast -> 用户查看并已读
适合原因：Laravel 官方能力足够覆盖学习项目
不足：通知聚合和多渠道退订需要额外设计
后续升级：聚合通知、邮件/Push、多渠道偏好设置
```
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 小型业务方案 | Laravel + MySQL + Redis，通知中心服务 单机运行 | 学习项目到小型站点 | 实现成本低，便于排查 | 容量和高可用有限 | 低 | 低 | 学习期推荐 |
| 中型业务方案 | Laravel Notification、database channel、broadcast channel、Redis unread、Queue、notifications 表，队列和缓存拆分，关键任务异步 | 有稳定用户和内容增长 | 吞吐更好，边界清晰 | 需要监控和运维 | 中 | 中 | 推荐演进 |
| 大型业务方案 | 通知中心服务 服务化，Redis Cluster，多 worker，多节点 WebSocket/媒体/存储 | 高并发平台 | 可横向扩展 | 架构复杂 | 高 | 高 | 只做设计理解 |
...
- 日互动事件量、粉丝规模、单用户未读数、通知保留周期、广播连接数
...
| 瓶颈 | 表现 | 原因 | 解决方式 |
| --- | --- | --- | --- |
| 大主播开播通知粉丝过多 | 请求变慢、数据不准或任务积压 | 设计不当或容量增长过快 | 拆分异步任务、增加缓存/索引、扩展 worker 或改用专门中间件 |
| 通知表增长快 | 请求变慢、数据不准或任务积压 | 设计不当或容量增长过快 | 拆分异步任务、增加缓存/索引、扩展 worker 或改用专门中间件 |
| 未读数频繁更新 | 请求变慢、数据不准或任务积压 | 设计不当或容量增长过快 | 拆分异步任务、增加缓存/索引、扩展 worker 或改用专门中间件 |
| 实时广播扇出 | 请求变慢、数据不准或任务积压 | 设计不当或容量增长过快 | 拆分异步任务、增加缓存/索引、扩展 worker 或改用专门中间件 |
...
如果这个模块从 Demo 变成生产系统，我会先拆清 Laravel 和中间件边界；同步请求只做必要校验和状态写入，耗时任务交给队列，高频状态交给 Redis，大文件和媒体流交给对象存储、CDN 或媒体服务器。容量估算重点看 日互动事件量、粉丝规模、单用户未读数、通知保留周期、广播连接数。流量上来后，优先处理 大主播开播通知粉丝过多、通知表增长快。
...
- notifications：Laravel 默认字段 type、notifiable_id、data、read_at
- notification_preferences：user_id、type、database_enabled、broadcast_enabled、email_enabled
- notification_aggregates：user_id、type、target_id、count、last_event_at
...
- NotificationDispatchService：统一发送入口
- UnreadNotificationService：未读数读取和校准
- NotificationAggregateService：聚合同类事件

### 8.5 Job

- SendDatabaseNotificationJob
- BroadcastNotificationJob
- AggregateNotificationsJob

Job 需要设置合理的 `tries`、`timeout`、`backoff`，失败后更新业务状态并写入可排查的错误摘要。

### 8.6 Event / Listener

- VideoLiked、CommentCreated、UserFollowed、LiveStarted、DirectMessageSent
...
| Laravel Reverb | laravel/reverb | 实时通知 | 后续扩展 |
| Mailpit | mailpit/mailpit | 邮件通知本地测试 | 可选 |
...
- unread:notifications:{userId} 计数
- notification dispatch 去重 key
- 开播通知粉丝列表分批缓存
...
- 实时通知走 private channel，只推摘要和未读数
- 断线后通过 HTTP 通知列表补齐

## 12. 安全风险

- 通知风暴：点赞聚合和采样
- 隐私泄露：广播只发给 notifiable 用户
- 重复通知：事件幂等 key
- 骚扰：通知偏好和退订
...
- Phase 1：database notification、通知列表、标记已读
- Phase 2：未读数缓存、实时广播、通知偏好
- Phase 3：聚合通知、多渠道发送、开播通知分批和退订
...
因为 用户无法感知互动反馈，关注开播和评论回复等闭环断开。，所以需要把业务状态、技术组件和异常处理拆清楚。
...
通知系统的核心是把业务事件异步转成用户可消费的信息，同时控制未读数、实时性和通知风暴。

## 一句话总结

通知系统的核心设计是：Laravel 管业务和状态，中间件管性能和基础设施，所有高频、耗时、可失败的部分都要异步化、可观测、可重试。
