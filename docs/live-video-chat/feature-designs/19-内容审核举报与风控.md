...
内容审核、举报与风控属于 **业务 + 技术混合模块**。

审核和风控包含视频、评论、直播、聊天、举报、封禁、审计日志和状态流转。

它涉及的核心组件是：moderation_reports、moderation_actions、blocked_words、user_bans、admin backend、Policy、Queue。

## 2. 解决的问题

- 用户层面：用户可以举报违规内容，管理员可以审核、下架、禁言或封禁。
- 系统层面：给平台提供内容安全闭环，避免违规内容长期公开。
- 不做的问题：主流平台必备的审核、举报、下架和封禁能力缺失，项目只能算技术 Demo。
- 所在位置：它连接后台管理、评论、聊天、视频发布和直播房间。

## 3. 常见方案对比

| 方案 | 核心思路 | 需要组件 | 优点 | 缺点 | 适合阶段 | 是否推荐 |
| --- | --- | --- | --- | --- | --- | --- |
| 人工后台审核 | 管理员查看举报并处理 | MySQL、Blade | 可控易学 | 效率有限 | 学习项目 | 推荐起步 |
| 敏感词规则 | 发布前命中规则自动拦截或待审 | blocked_words | 成本低 | 误杀漏判 | 学习项目 | 推荐 |
| 机审 + 人审 | AI/云审核初筛，人审复核 | 云审核 API、Queue | 生产常见 | 成本和接入复杂 | 生产环境 | 后续 |
| 实时风控系统 | 行为评分、设备/IP、黑名单联动 | 风控服务 | 适合大平台 | 复杂 | 大型平台 | 当前不做 |
...
```text
方案名称：举报表 + 审核动作表 + 敏感词 + 管理后台
核心组件：MySQL、Policy、Queue、Blade admin、Redis RateLimiter
Laravel 职责：收集举报、审核状态流转、执行下架/禁言/封禁、审计日志
中间件职责：Redis 限流举报和敏感词缓存，队列处理通知
数据流：用户举报 -> 生成 report -> 管理员处理 -> 写 action -> 更新目标状态 -> 通知相关用户
适合原因：先做规则和流程即可贴近真实平台
不足：不含复杂机审、版权识别和黑产对抗
后续升级：云审核、审核队列分派、风控评分
```
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 小型业务方案 | Laravel + MySQL + Redis，审核风控服务 单机运行 | 学习项目到小型站点 | 实现成本低，便于排查 | 容量和高可用有限 | 低 | 低 | 学习期推荐 |
| 中型业务方案 | moderation_reports、moderation_actions、blocked_words、user_bans、admin backend、Policy、Queue，队列和缓存拆分，关键任务异步 | 有稳定用户和内容增长 | 吞吐更好，边界清晰 | 需要监控和运维 | 中 | 中 | 推荐演进 |
| 大型业务方案 | 审核风控服务 服务化，Redis Cluster，多 worker，多节点 WebSocket/媒体/存储 | 高并发平台 | 可横向扩展 | 架构复杂 | 高 | 高 | 只做设计理解 |
...
- 日举报量、管理员处理能力、待审内容量、敏感词数量、审核日志保留期
...
| 瓶颈 | 表现 | 原因 | 解决方式 |
| --- | --- | --- | --- |
| 人工审核积压 | 请求变慢、数据不准或任务积压 | 设计不当或容量增长过快 | 拆分异步任务、增加缓存/索引、扩展 worker 或改用专门中间件 |
| 敏感词匹配慢 | 请求变慢、数据不准或任务积压 | 设计不当或容量增长过快 | 拆分异步任务、增加缓存/索引、扩展 worker 或改用专门中间件 |
| 后台列表筛选慢 | 请求变慢、数据不准或任务积压 | 设计不当或容量增长过快 | 拆分异步任务、增加缓存/索引、扩展 worker 或改用专门中间件 |
| 封禁状态多入口遗漏 | 请求变慢、数据不准或任务积压 | 设计不当或容量增长过快 | 拆分异步任务、增加缓存/索引、扩展 worker 或改用专门中间件 |
...
如果这个模块从 Demo 变成生产系统，我会先拆清 Laravel 和中间件边界；同步请求只做必要校验和状态写入，耗时任务交给队列，高频状态交给 Redis，大文件和媒体流交给对象存储、CDN 或媒体服务器。容量估算重点看 日举报量、管理员处理能力、待审内容量、敏感词数量、审核日志保留期。流量上来后，优先处理 人工审核积压、敏感词匹配慢。
...
sequenceDiagram
    ...
    participant C as 审核风控服务

    U->>B: 发起内容审核、举报与风控相关操作
    B->>L: 请求接口
    L->>L: 鉴权、校验、状态判断
    L->>R: 读写缓存、限流或临时状态
    L->>Q: 派发异步任务
...
- moderation_reports：reporter_id、target_type、target_id、reason、status
- moderation_actions：admin_id、target_type、target_id、action、reason
- blocked_words：word、scope、action
- user_bans：user_id、reason、expires_at、status

### 8.2 Model

- ModerationReport morphTo target
- ModerationAction morphTo target
- UserBan belongsTo User

### 8.3 Controller

- ReportController：store
- AdminModerationController：index/approve/reject/action
- BlockedWordController：manage

Controller 只负责接收请求、调用授权、组织响应；复杂状态、文件、队列和中间件调用应放到 Service 或 Job。

### 8.4 Service / Support / Action

- ModerationService：处理举报和动作
- BlockedWordService：内容命中判断
- UserBanService：封禁状态判断
- AuditLogService：审计

### 8.5 Job

- NotifyModerationResultJob
- ScanPendingContentJob
- ExpireUserBanJob
...
- ContentReported、ModerationActionTaken、UserBanned、ContentBlocked

### 8.7 WebSocket / Broadcasting

- private-admin.moderation 推送新举报
- private-user.{id} 推送处理结果

### 8.8 Policy / Middleware

- 登录用户可举报；管理员/审核员可处理；被封禁用户限制发布、评论和聊天
...
| Redis | redis:7-alpine | 限流和敏感词缓存 | 需要 |
| Mailpit | mailpit/mailpit | 审核结果通知 | 可选 |
...
- Phase 1：举报、后台处理、审核动作日志
- Phase 2：敏感词、禁言/封禁、视频/评论/直播联动
- Phase 3：云审核、风控评分、审核效率指标
...
因为 主流平台必备的审核、举报、下架和封禁能力缺失，项目只能算技术 Demo。，所以需要把业务状态、技术组件和异常处理拆清楚。
...
审核风控要先建立流程闭环：用户能举报，管理员能处理，系统能执行状态变化，并且所有动作可审计、可追踪。

## 一句话总结

内容审核、举报与风控的核心设计是：Laravel 管业务和状态，中间件管性能和基础设施，所有高频、耗时、可失败的部分都要异步化、可观测、可重试。
